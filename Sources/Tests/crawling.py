...
class Crawler:
    ...
    # 保存图片
    def save_image(self, rsp_data, word,prefix=""):
        ...
        for image_info in rsp_data['data']:
            try:
                ...
                # 保存图片
                filepath = './%s/%s' % (word,str(prefix)+ str(self.__counter) + str(suffix))
    ...
    # 开始获取
    def get_images(self, word,prefix):
        ...
        while pn < self.__amount:
            ...
            try:
                ...
                self.save_image(rsp_data, word,prefix)
                # 读取下一页
                print("下载下一页")
                pn += 60
            finally:
    ...
    def start(self, word, total_page=1, start_page=1, per_page=30,prefix=""):
        """
        爬虫入口
        :param word: 抓取的关键词
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        crawler.start('风景', 1, 1, 30,"fj_")  # 抓取关键词为 “美女”，总数为 1 页，开始页码为 2，每页30张（即总共 2*30=60 张）
